...
# unit test to show we can get the final output that is correctly feature engineered
def test_clean_education():
    df["EDUCATION"] = clean_education_level(df)
    for row in df.index:
        assert (
            df.loc[row, "EDUCATION"] != 6
        ), """Cleaning Education
        """
